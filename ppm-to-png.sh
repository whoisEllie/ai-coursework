...
if [ "$height" -gt 640 ]
then
    ffmpeg -i $1 -vf scale=-1:-1:flags=neighbor $2
elif [ "$height" -gt 320 ]
then
    ffmpeg -i $1 -vf scale=4*iw:4*ih:flags=neighbor $2
elif [ "$height" -gt 80 ]
then
    ffmpeg -i $1 -vf scale=8*iw:8*ih:flags=neighbor $2
else
    ffmpeg -i $1 -vf scale=32*iw:32*ih:flags=neighbor $2
fi
